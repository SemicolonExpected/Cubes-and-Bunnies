...
Vector2i isInsideTriangle2(MatrixXi objects, MatrixXf vertices, MatrixXi indices,
		   MatrixXf rot, MatrixXf scale, MatrixXf translate,
		   Vector3f w_rot, float w_scale,
		   Vector2f point, int ifPerspective){

  int objNumber = -1; float maxZ = -3; int triangleIndex = -1;
  MatrixXf v(3,3); float minD = 3;
  ...
  for(int i = 0; i < objects.cols(); i++){
	...
	v3 = v3*(v3(2) / 0.5 + 1.0);
      ...
      Vector3f raydir(point(0),point(1),-1);
      float d = normal.dot(v1.transpose());
      float t = (-d)/normal.dot(raydir); //suppose to be -(normal.dot(origin) + d)/normal.dot(raydir) but since origin is a 0 vector it just = 0

      if(t < minD){ //if its farther than the current closest don't bother checking if inside triangle because it also doesn't matter
	 //check vertices
	float a,b,c;
	a = ( ((v2(1)+v3(1))*(point(0)-v3(0)))+((v3(0)-v2(0))*(point(1)-v3(1))) )/
	  ( ((v2(1)+v3(1))*(v1(0)   -v3(0)))+((v3(0)-v2(0))*(v1(1)-v3(1))) );
	...
	Vector2f v12(v1(0),v1(1)); Vector2f v22(v2(0),v2(1)); Vector2f v32(v3(0),v3(1));
	//if hit
	if(a > 0 && b > 0 && c > 0){
	  //distanceToPlane = normal.dot(v1.transpose());
	  //distanceToPixel = (normal.dot(origin)+ distanceToPlane)/normal.dot(ray_direction); //distance from ray origin to pixel t
	  //check z value to maxZ if bigger then update Obj num and max Z
	    triangleIndex = j;
  ...
  Vector2i objTrianglePair(objNumber, triangleIndex);
  return objTrianglePair;
}
